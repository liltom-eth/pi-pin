...
def main():
    shell.clear()
    print("This script downloads and installs I2S microphone support.")
    ...
    auto_load = not shell.argument_exists("noautoload") and shell.prompt(
        "Auto load module at boot?", force_arg="autoload"
    )
    ...
    if auto_load:
        shell.write_text_file(
            "/etc/modules-load.d/snd-i2smic-rpi.conf", "snd-i2smic-rpi"
        )
        shell.write_text_file(
